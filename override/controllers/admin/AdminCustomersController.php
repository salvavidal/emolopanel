...
class AdminCustomersController extends AdminCustomersControllerCore
{
    ...
    public function initPageHeaderToolbar()
    {
        ...
        if (Tools::getValue('id_customer')) {
            $this->page_header_toolbar_btn['login_as_customer'] = [
                'href' => self::$currentIndex . '&id_customer=' . Tools::getValue('id_customer') . '&action=loginAsCustomer&token=' . $this->token,
                'desc' => $this->l('Login como Cliente'),
                'icon' => 'process-icon-users'
            ];
        }
    }
    ...
    {
        if (Tools::getValue('action') === 'loginAsCustomer') {
